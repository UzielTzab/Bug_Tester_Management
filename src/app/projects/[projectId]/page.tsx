...
export default function ProjectDashboard() {
  ...
  const projectIdFromUrl = params?.projectId ? (Array.isArray(params.projectId) ? params.projectId[0] : params.projectId) : DEFAULT_PROJECT_ID;
  ...
  const [currentProjectId, setCurrentProjectId] = useState<string>(projectIdFromUrl);
    ...
    const runInitialization = async () => {
      if (!migrationDone) {
        try {
          ...
          const existingProjects: Project[] = projectsResponse.ok ? await projectsResponse.json() : [];
  ...
  // Cargar viewMode del localStorage
  useEffect(() => {
    try {
      const savedViewMode = localStorage.getItem('bug-tracker-view-mode') as 'cards' | 'list' | 'kanban' | null;
      if (savedViewMode && ['cards', 'list', 'kanban'].includes(savedViewMode)) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('bug-tracker-view-mode', viewMode);
    } catch (error) {
  ...
  const fetchProjectCount = async (projectId: string): Promise<number> => {
    try {
  ...
  const refreshAllProjectCounts = async (projectList: Project[]) => {
    const countEntries = await Promise.all(
      projectList.map(async (project) => [project.id, await fetchProjectCount(project.id)] as const)
    );
    setProjectBugCounts(Object.fromEntries(countEntries));
  };

  const loadProjects = async () => {
    try {
      const response = await fetch('/api/projects');
      const loadedProjects: Project[] = response.ok ? await response.json() : [];
      setProjects(loadedProjects);
      await refreshAllProjectCounts(loadedProjects);
    } catch (error) {
  ...
  /**
   * Remover actor del proyecto siendo creado
   */
  const handleRemoveActorCreate = (index: number) => {
    setNewProjectData(prev => ({
      ...prev,
      actors: prev.actors.filter((_, i) => i !== index)
    }));
  ...
  const handleDeleteProject = async () => {
    ...
    setIsLoading(true);
    try {
      const projectName = projects.find(p => p.id === editingProjectId)?.name || 'Proyecto';
      
      await fetch(`/api/projects/${editingProjectId}`, {
        method: 'DELETE',
      });
      
      // Redirigir a home con parámetro de éxito
      router.push(`/?projectDeleted=${encodeURIComponent(projectName)}`);
    } catch (error) {
  ...
  const fetchRecords = async () => {
    ...
    try {
      const res = await fetch(`/api/records?projectId=${encodeURIComponent(currentProjectId)}`);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const count = await fetchProjectCount(currentProjectId);
      setProjectBugCounts((prev) => ({ ...prev, [currentProjectId]: count }));
      resetForm();
      setShowForm(false);
    } catch (error) {
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      await refreshAllProjectCounts(projects);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row overflow-x-hidden" style={{ backgroundColor: 'var(--bg-light-gray)' }}>
      ...
      <div className="w-full md:w-64 bg-gradient-to-b from-green-600 to-blue-700 border-b md:border-r border-blue-800 p-4 md:p-6 md:fixed md:h-screen md:overflow-y-auto shadow-lg">
        ...
        <div className="space-y-3 mb-6">
          {currentProject ? (
            ...
            <p className="text-sm text-blue-100 p-3 bg-blue-500 rounded-lg">
              Cargando proyecto...
            </p>
          )}
        </div>
      ...
      <div className="w-full md:flex-1 md:ml-64 p-4 md:p-6 overflow-x-hidden grid-paper-bg">
        ...
        {imageModal && (
          <div
            className="fixed inset-0 bg-black/10 backdrop-blur-md flex items-center justify-center z-[9999] p-4"
        ...
        >
          <div className="space-y-4">
            <p className="text-gray-700">
              ¿Estás seguro de que deseas eliminar el proyecto <strong>{editingProjectData.name}</strong>? Esta acción no se puede deshacer.
            </p>
            <div className="flex gap-3">
              <Button variant="secondary" className="flex-1" onClick={() => setShowDeleteProjectModal(false)}>
                Cancelar
              </Button>
              <Button variant="danger" className="flex-1" onClick={handleDeleteProject} loading={isLoading}>
                Eliminar Proyecto
              </Button>
            </div>
          </div>
